...
const char auth[] = "XXXXXXXXXXXXXXXXXXX"; //Blynkと連携するためのトークン宣言
const char ssid[] = "XXXXXXXXXXXXXXXXXXX"; //wifiネットワークのSSID宣言
const char pass[] = "XXXXXXXXXXXXXXXXXXX"; //wifiネットワークのパスワード宣言
const char address[] = "XXXXXXXXXXXXXXXX"; //通知のためのメールアドレス宣言
...
BLYNK_WRITE(V1)
{
  int v1Param = param[0].asInt(); //V0ピンの値をv1Paramに格納
  M5.Lcd.print(v1Param); //アプリ側で操作した値をLCDに表示
}
...
void buttonCheck()
{
  ...
  {
    Blynk.email(address, "setup", "Your message goes here"); //ボタンが押された時にgmailにメッセージを送る
    M5.Lcd.println("A");
  }
  if (M5.BtnB.wasReleased())
  {
    //M5.Lcd.println("B");
    ClockDisplay(); //Bボタンを押したときにLCDに時刻と日付を出力
  }
